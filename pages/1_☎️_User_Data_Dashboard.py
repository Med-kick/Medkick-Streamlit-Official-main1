...
def main():
    ...
    # Function to check if a nurse met the criteria for an inbound call
    def evaluate_performance_x(row):
        try:
            from_component, to_component = extract_call_components(row)
            if row['Direction'] == 'INBOUND':
                answer_time_str = row['Answer Time (local)']
                ...
                if answer_time_str == '--:--:--':
                    ...
                    # Look for an outbound call initiated by the nurse to the patient
                    call_back_row_nurse_in_x_time = df[(df['Direction'] == 'OUTBOUND') & 
                                       (df['To'].apply(lambda x: re.sub(r'\D', '', x)[-10:]) == from_number[-10:]) & 
                                       (df['From'].apply(lambda x: re.sub(r'\D', '', x)) == to_number) & 
                                       (row['Start Time (local)'] - pd.Timedelta(seconds=10) <= df['Start Time (local)']) & 
                                       (df['Start Time (local)'] <= row['Start Time (local)'] + pd.Timedelta(seconds=x)) & 
                                       (df['Duration'] >= 1)].head(1)
                
                    # Look for an inbound call initiated by the patient to the nurse
                    call_back_row_patient_in_x_time = df[(df['Direction'] == 'INBOUND') & 
                                       (df['To'].apply(lambda x: re.sub(r'\D', '', x)[-10:]) == to_number) & 
                                       (df['From'].apply(lambda x: re.sub(r'\D', '', x)) == from_number[-10:]) & 
                                       (row['Start Time (local)'] - pd.Timedelta(seconds=10) <= df['Start Time (local)']) & 
                                       (df['Start Time (local)'] <= row['Start Time (local)'] + pd.Timedelta(seconds=x)) & 
                    ...
                    if not call_back_row_nurse_in_x_time.empty:
                        # Ensure that the nurse called back and it's an outbound call
                        if call_back_row_nurse_in_x_time['Direction'].values[0] == 'OUTBOUND':
                            call_back_time_in_x_time = pd.to_datetime(call_back_row_nurse_in_x_time['Start Time (local)'].values[0]).tz_localize(None)
                            duration_call_back_in_x_time = call_back_row_nurse_in_x_time['Duration'].values[0]
                            return f'Met criteria. Did not answer. Nurse called back in',selected_duration_threshold,' seconds.'
                        else:
                            return f'Did not answer. Nurse did not call back in',selected_duration_threshold,' seconds.'
                    elif not call_back_row_patient_in_x_time.empty:
                        # Ensure that the patient called back and it's an inbound call
                        if call_back_row_patient_in_x_time['Direction'].values[0] == 'INBOUND':
                            call_back_time_in_x_time = pd.to_datetime(call_back_row_patient_in_x_time['Start Time (local)'].values[0]).tz_localize(None)
                            duration_call_back_in_x_time = call_back_row_patient_in_x_time['Duration'].values[0]
                            return f'Met criteria. Did not answer. Patient called back in',selected_duration_threshold,' seconds.'
                        else:
                            return f'Did not answer. Nurse did not call back in',selected_duration_threshold,' seconds.'
                    else:
                        return f'Did not answer. Nurse did not call back in 10hr.'
    ...
    selected_duration_threshold = st.sidebar.slider('Select Duration Threshold', min_value=1, max_value=2592000, value=360)  # You can adjust the min, max, and initial value accordingly
    ...
    if not selected_from_date or not selected_to_date:
        ...
        # Filter DataFrame based on selected date range
        filtered_df_x = df[(df['Start Time (local)'].dt.date >= selected_from_date) & (df['Start Time (local)'].dt.date <= selected_to_date)]

        # Apply the function to the filtered DataFrame
        filtered_df_x['Performance'] = filtered_df_x.apply(evaluate_performance_x, axis=1)

        # Count the number of calls that meet and do not meet the criteria
        met_criteria_count_x = len(filtered_df_x[filtered_df_x['Performance'].str.startswith('Met')])
        Answer_Call_count_x = len(filtered_df_x[filtered_df_x['Performance'].str.startswith('Met criteria. Answered')])
        Missed_Call_count_x = len(filtered_df_x[filtered_df_x['Performance'].str.startswith(('Met criteria. Did not answer.', 'Did not'))])
        Nurse_Call_Back_count_x = len(filtered_df_x[filtered_df_x['Performance'].str.startswith('Met criteria. Did not answer. Nurse called back')])
        Patient_Call_Back_count_x = len(filtered_df_x[filtered_df_x['Performance'].str.startswith('Met criteria. Did not answer. Patient called back')])
        did_not_meet_criteria_count_x = len(filtered_df_x[filtered_df_x['Performance'].str.startswith('Did not')])
        
        # Count the number of inbound and outbound calls
        inbound_call_count_x = len(filtered_df_x[filtered_df_x['Direction'] == 'INBOUND'])
        outbound_call_count_x = len(filtered_df_x[filtered_df_x['Direction'] == 'OUTBOUND'])

        # Display the counts on top of the dashboard
        st.markdown(f"<p style='color:#5cb7a2;font-size:20px;'> Outbound Calls: <strong>{outbound_call_count_x}</strong></p>", unsafe_allow_html=True)
        st.markdown(f"<p style='color:#5cb7a2;font-size:20px;'> Inbound Calls: <strong>{inbound_call_count_x}</strong></p>", unsafe_allow_html=True)
        st.markdown(f"<p style='color:green;font-size:20px;'> &nbsp;&nbsp;&nbsp;&nbsp;Answered calls: <strong>{Answer_Call_count_x}</strong></p>", unsafe_allow_html=True)
        st.markdown(f"<p style='color:orange;font-size:20px;'> &nbsp;&nbsp;&nbsp;&nbsp;Missed calls: <strong>{Missed_Call_count_x}</strong></p>", unsafe_allow_html=True)
        st.markdown(f"<p style='color:red;font-size:20px;'> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;No Follow-up: <strong>{did_not_meet_criteria_count_x}</strong></p>", unsafe_allow_html=True)
        st.markdown(f"<p style='color:#D8D800;font-size:20px;'> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Follow-up <1 hr: <strong>{Nurse_Call_Back_count_x + Patient_Call_Back_count_x}</strong></p>", unsafe_allow_html=True)
        st.markdown(f"<p style='color:#D8D800;font-size:18px;'> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;By Nurse: <strong>{Nurse_Call_Back_count_x}</strong></p>", unsafe_allow_html=True)
        st.markdown(f"<p style='color:#D8D800;font-size:18px;'> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;By Patient: <strong>{Patient_Call_Back_count_x}</strong></p>", unsafe_allow_html=True)
        ...
        if performance_criteria == 'All':
            # Display only specific columns
            display_columns = ['From', 'To', 'Start Time (local)', 'Duration', 'Direction', 'Performance']
            st.write(filtered_df_x[display_columns])
        elif performance_criteria == 'Answered':
            # Display only specific columns
            display_columns = ['From', 'To', 'Start Time (local)', 'Duration', 'Direction', 'Performance']
            st.write(filtered_df_x[filtered_df_x['Performance'].str.startswith('Met criteria. Answered')][display_columns])
        elif performance_criteria == 'Missed':
            # Display only specific columns
            display_columns = ['From', 'To', 'Start Time (local)', 'Duration', 'Direction', 'Performance']
            st.write(filtered_df_x[filtered_df_x['Performance'].str.startswith('Met criteria. Did not answer.') | filtered_df_x['Performance'].str.startswith('Did not')])
        elif performance_criteria == 'No Follow-up':
            # Display only specific columns
            display_columns = ['From', 'To', 'Start Time (local)', 'Duration', 'Direction', 'Performance']
            st.write(filtered_df_x[filtered_df_x['Performance'].str.startswith('Did not')][display_columns])
        elif performance_criteria == 'Follow-up >1hr and <10hr':
            # Display only specific columns
            display_columns = ['From', 'To', 'Start Time (local)', 'Duration', 'Direction', 'Performance']
            st.write(filtered_df_x[filtered_df_x['Performance'].str.startswith('Did not Met criteria. Did not answer. Nurse called back') | filtered_df['Performance'].str.startswith('Did not Met criteria. Did not answer. Patient called back')])
        elif performance_criteria == 'Follow-up <1hr':
            # Display only specific columns
            display_columns = ['From', 'To', 'Start Time (local)', 'Duration', 'Direction', 'Performance']
            st.write(filtered_df_x[filtered_df_x['Performance'].str.startswith('Met criteria')][display_columns])
